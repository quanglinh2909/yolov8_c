...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results, int src_img_width, int src_img_height)
{
    ...
    if (validCount <= 0)
    {
        // printf("No objects detected (validCount=%d, conf_threshold=%.2f)\n", validCount, conf_threshold);
        return 0;
    }
    // printf("Detected %d objects before NMS (conf_threshold=%.2f, nms_threshold=%.2f)\n", validCount, conf_threshold, nms_threshold);
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        // Filter by class if DETECT_CLASS_FILTER array is not empty
        if (DETECT_CLASS_FILTER_COUNT > 0) {
            bool class_found = false;
            for (int j = 0; j < DETECT_CLASS_FILTER_COUNT; j++) {
                if (id == DETECT_CLASS_FILTER[j]) {
                    class_found = true;
                    break;
                }
            }
            if (!class_found) {
                continue;  // Skip this detection if it's not in the filter list
            }
        }

        // Convert coordinates back to original image space
        image_rect_t final_box;
        final_box.left = (int)(clamp(x1, 0, model_in_w) / letter_box->scale);
        final_box.top = (int)(clamp(y1, 0, model_in_h) / letter_box->scale);
        final_box.right = (int)(clamp(x2, 0, model_in_w) / letter_box->scale);
        final_box.bottom = (int)(clamp(y2, 0, model_in_h) / letter_box->scale);
        
        // Filter by ROI - only keep detections inside defined zones
        if (!is_box_in_roi(&final_box, src_img_width, src_img_height)) {
            continue;  // Skip this detection if it's outside all ROI zones
        }

        od_results->results[last_count].box = final_box;
        od_results->results[last_count].prop = obj_conf;
        od_results->results[last_count].cls_id = id;
        last_count++;
    }
    od_results->count = last_count;
    // printf("Final detected objects after NMS: %d\n", last_count);
    return 0;
}

// Global ROI zones storage
roi_zone_t g_roi_zones[MAX_ROI_ZONES];
int g_roi_zones_count = 0;
int g_roi_overlap_percent = 0;  // 0 = center point, 1-100 = % overlap

// Helper: Calculate rectangle intersection area
static int rect_intersection_area(int ax1, int ay1, int ax2, int ay2,
                                   int bx1, int by1, int bx2, int by2)
{
    int x_left = (ax1 > bx1) ? ax1 : bx1;
    int y_top = (ay1 > by1) ? ay1 : by1;
    int x_right = (ax2 < bx2) ? ax2 : bx2;
    int y_bottom = (ay2 < by2) ? ay2 : by2;
...
// Helper: Check if point is inside rectangle ROI
static bool point_in_rect_roi(int x, int y, const roi_zone_t* zone, int img_width, int img_height)
{
    float norm_x = (float)x / img_width;
    float norm_y = (float)y / img_height;
    
    return (norm_x >= zone->x && norm_x <= (zone->x + zone->width) &&
            norm_y >= zone->y && norm_y <= (zone->y + zone->height));
}

// Point-in-polygon test using ray casting algorithm
static bool point_in_polygon(int x, int y, const roi_point_t* polygon, int num_points)
{
    bool inside = false;
    int j = num_points - 1;
...
// Calculate overlap percentage between box and polygon ROI
static int calc_polygon_overlap_percent(const image_rect_t* box, const roi_zone_t* zone)
{
    // Sample grid approach: check how many points inside polygon
    int samples_x = 10;
    int samples_y = 10;
    int inside_count = 0;
    int total_samples = samples_x * samples_y;
    ...
    for (int sy = 0; sy < samples_y; sy++) {
        for (int sx = 0; sx < samples_x; sx++) {
            int sample_x = box->left + (box_width * sx) / samples_x;
            int sample_y = box->top + (box_height * sy) / samples_y;
            
            if (point_in_polygon(sample_x, sample_y, zone->points, zone->num_points)) {
                inside_count++;
            }
        }
    }
    
    return (inside_count * 100) / total_samples;
}

// Check if a box is inside any ROI zone
bool is_box_in_roi(const image_rect_t* box, int img_width, int img_height)
{
    // If no ROI zones defined, accept all boxes (full frame)
    if (g_roi_zones_count == 0) {
        return true;
    }
    
    // Mode 0: Check center point only (fast)
    if (g_roi_overlap_percent == 0) {
        int box_center_x = (box->left + box->right) / 2;
        int box_center_y = (box->top + box->bottom) / 2;
        
        for (int i = 0; i < g_roi_zones_count; i++) {
            const roi_zone_t* zone = &g_roi_zones[i];
            
            if (zone->type == ROI_TYPE_RECT) {
                if (point_in_rect_roi(box_center_x, box_center_y, zone, img_width, img_height)) {
                    return true;
                }
            } else if (zone->type == ROI_TYPE_POLYGON) {
                if (point_in_polygon(box_center_x, box_center_y, zone->points, zone->num_points)) {
                    return true;
                }
            }
        }
    }
    // Mode 1-100: Check percentage overlap
    else {
        int box_area = (box->right - box->left) * (box->bottom - box->top);
        ...
        for (int i = 0; i < g_roi_zones_count; i++) {
            ...
            if (zone->type == ROI_TYPE_RECT) {
                ...
                int roi_x1 = (int)(zone->x * img_width);
                int roi_y1 = (int)(zone->y * img_height);
                int roi_x2 = (int)((zone->x + zone->width) * img_width);
                int roi_y2 = (int)((zone->y + zone->height) * img_height);
                
                int overlap_area = rect_intersection_area(
                    box->left, box->top, box->right, box->bottom,
                    roi_x1, roi_y1, roi_x2, roi_y2
                );
                
                int overlap_percent = (overlap_area * 100) / box_area;
                if (overlap_percent >= g_roi_overlap_percent) {
                    return true;
                }
                
            } else if (zone->type == ROI_TYPE_POLYGON) {
                // Calculate polygon overlap (sampling method)
                int overlap_percent = calc_polygon_overlap_percent(box, zone);
                if (overlap_percent >= g_roi_overlap_percent) {
                    return true;
                }
            }
