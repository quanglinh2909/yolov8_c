...
void tracker_update(Tracker* tracker, object_detect_result* detections, int num_detections,
    ...
    tracker->current_frame++;  // Increment frame counter
    ...
    for (int i = 0; i < tracker->count; i++) {
        ...
        for (int j = 0; j < num_detections; j++) {
            ...
            // Different confidence thresholds based on track state:
            // - TENTATIVE: Require HIGH confidence (BOX_THRESH) during confirmation phase
            // - NEW/TRACKED/LOST_TEMP: Accept LOW confidence (BOX_THRESH_TRACKING) to maintain track
            float min_confidence = (track->state == TRACK_STATE_TENTATIVE) ? BOX_THRESH : BOX_THRESH_TRACKING;
            if (detections[j].prop < min_confidence) continue;
        ...
        if (best_det_idx >= 0) {
            ...
            track->age++;
            track->hits++;  // Increment hits within time window
            
            // Check if enough time has passed or enough hits collected
            int frames_elapsed = tracker->current_frame - track->first_seen_frame;
            
            // State transitions based on hits within time window
            if (track->state == TRACK_STATE_TENTATIVE) {
            ...
            // If tentative track is lost, remove it immediately
            if (track->state == TRACK_STATE_TENTATIVE) {
                track->state = TRACK_STATE_LOST_PERMANENT;
                // Don't notify for tentative tracks
            } else if (track->time_since_update > MAX_DISAPPEARED_FRAMES) {
                track->state = TRACK_STATE_LOST_PERMANENT;
    ...
    for (int j = 0; j < num_detections; j++) {
        ...
        // Only create NEW tracks for high confidence detections (BOX_THRESH)
        // This prevents creating tracks from noise/false positives
        ...
        if (slot >= 0) {
            ...
            new_track->state = TRACK_STATE_TENTATIVE;  // Start as tentative
            ...
            new_track->hits = 1;  // First hit
            new_track->first_seen_frame = tracker->current_frame;  // Record when first seen
            
            get_box_center(&new_track->box, &new_track->prev_cx, &new_track->prev_cy);
            
            // Only log if it will be confirmed immediately (hits >= MIN_HITS_TO_CONFIRM)
            // Otherwise wait until confirmed
            if (MIN_HITS_TO_CONFIRM <= 1) {
                new_track->state = TRACK_STATE_NEW;
                printf("✨ ID %d: NEW (immediate, min_hits=%d)\n", 
                       new_track->track_id, MIN_HITS_TO_CONFIRM);
                if (callback) callback(new_track->track_id, TRACK_STATE_LOST_PERMANENT, TRACK_STATE_NEW);
            } else {
                printf("👁️  ID %d: TENTATIVE (needs %d hits in %d frames)\n", 
                       new_track->track_id, MIN_HITS_TO_CONFIRM, CONFIRMATION_TIME_WINDOW);
            }
        }
    }
}
