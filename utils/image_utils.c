...
static int read_image_jpeg(const char* path, image_buffer_t* image)
{
    ...
    ret = tjDecompress2(handle, jpegBuf, size, sw_out_buf, width, 0, height, TJPF_RGB, flags);
...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        // Need to fill padding area with background color
        // Use CPU memset instead of RGA hardware fill to avoid issues
        if (dst != NULL) {
            size_t dst_size = get_image_size(dst_img);
            // printf("fill dst image (x y w h)=(%d %d %d %d) with color=0x%02x (CPU memset)\n",
            //        0, 0, dstWidth, dstHeight, color);
            memset(dst, color, dst_size);
        } else {
            // Try RGA hardware fill only if dst is not available
            ...
            if (ret_rga <= 0) {
                // Silently ignore RGA fill errors, letterbox will still work
                // printf("Note: RGA hardware fill not available, letterbox padding may not be filled\n");
            }
        }
    }
